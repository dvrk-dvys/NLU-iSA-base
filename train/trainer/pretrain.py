...
from transformers import (
    ...
    AutoTokenizer,
    BertTokenizer, DistilBertTokenizerFast, EncoderDecoderModel,
...
def collate_fn(batch):
    ...
    #                   _x[2].to(device=torch.device('mps')), _x[3], _x[4])
...
class pretrain_Lite(LightningModule):
    def __init__(
        ...
        if config['state_dict'] != '':
            ...
            self.decoder = self.decoder.to(device=torch.device('mps'))
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Generator Load
            generator_dict = torch.load(config['gen_state_dict'])
            self.generator.load_state_dict(state_dict=generator_dict)
            self.generator = self.generator.to(device=torch.device('mps'))
    ...
    def configure_optimizers(self):
        ...
        # optimizer_bert.optimizer.accelerator_state.device = torch.device('mps')
        # # optimizer_bert.optimizer.accelerator_state.device = 'mps'
        # optimizer_decoder.optimizer.accelerator_state.device = torch.device('mps')
        # # optimizer_bert.optimizer.accelerator_state.device = 'mps'
        # optimizer_generator.optimizer.accelerator_state.device = torch.device('mps')
        # # optimizer_bert.optimizer.accelerator_state.device = 'mps'
    ...
    #     #     for out_labels in output["labels"].detach().to(torch.device('mps')):
...
#     )
#     generator = Generator(len(tokenizer.vocab), config['decoder_hidden'], tokenizer.vocab['[PAD]'])
...
#         dist.barrier()
#         # configure map_location properly
#         map_location = {'mps:%d' % 0: 'mps:%d' % config['local_rank']}
#         # map_location = {'cuda:%d' % 0: 'cuda:%d' % config['local_rank']}
#         model_dict = torch.load(config['cache'], map_location=map_location)
#         decoder_dict = torch.load(config['decoder_cache'], map_location=map_location)
#         generator_dict = torch.load(config['generator_cache'], map_location=map_location)
